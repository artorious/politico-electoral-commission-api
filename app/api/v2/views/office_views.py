...
def create_an_office():
    ...
    if len(office_reg_data) > 2:
        custom_response = jsonify(sample_office.more_data_fields_response), 400
    elif len(office_reg_data) < 2:
        custom_response = jsonify(sample_office.few_data_fields_response), 400
...
def office_fetch(oid):
    ...
    if oid >= 1:
        if DatabaseManager().lookup_whether_entity_exists_in_a_table_by_attrib(
            ...
            raw_office = DatabaseManager().fetch_a_record_by_id_from_a_table(
                "offices", "oid", oid
            )
            office = {}
